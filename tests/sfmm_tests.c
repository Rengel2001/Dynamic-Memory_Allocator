...
//Test 1 will assert that a call to realloc with align parameter 0 is invalid
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	void* malloc1 = sf_malloc(40);
	void* realloc1 = sf_realloc(malloc1, 0);
	cr_assert_null(realloc1, "realloc return value is not NULL");
}

//Test 2 asserts that a call to realloc of the same size is valid
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT){
	void* malloc1 = sf_malloc(220);
	void* realloc1 = sf_realloc(malloc1, 220);
	cr_assert_eq(malloc1, realloc1, "expected return value of: %x, but we got: %x", malloc1, realloc1);
}

//Test 3 asserts that an alignment request of 8 is not null
Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT) {
	void* x = sf_memalign(64, 8);
	cr_assert_not_null(x, "x is NULL, got: %p", x);
}

//Test 4 asserts that the alignment requirement is a power of 2
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT){
	void* x = sf_memalign(80, 17);
	cr_assert_null(x, "x is not NULL, got: %p", x);
	cr_assert_eq(sf_errno, EINVAL, "sf_errno should be einval but we got: %d, instead of %d)", sf_errno, EINVAL);
}

//Test 5 asserts that the alignment requirement is greater than 8
Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT){
	void* x = sf_memalign(80, 4);
	cr_assert_null(x, "x is not NULL, got: %p", x);
	cr_assert_eq(sf_errno, EINVAL, "sf_errno should be einval but we got: %d, instead of %d)", sf_errno, EINVAL);
}
